...
# Fill in the lines labeled YOUR_INSTRUCTIONS_HERE according to the descriptions
# above. Each blank may require one or more MIPS instructions.
#
# Arguments:
...
write_machine_code_find_text:	
	...
	# blanks below with the correct instruction:
	
	# 1. Initialize the byte offset to zero. We will need this for any instructions
	# that require relocation:
	# YOUR_INSTRUCTIONS_HERE
	li $s5, 0				# $s5 = byte offset
	# END_INSTRUCTIONS_HERE
write_machine_code_next_inst:
	# 2. Call readline() while passing in the correct arguments:
	# YOUR_INSTRUCTIONS_HERE
	move $a0, $s1			# $a1 = input file
	jal readline
	# END_INSTRUCTIONS_HERE
	blt $v0, $0, write_machine_code_error
	move $s4, $v0			# $s4 = whether EOF was reached
	#Check whether there are any characters besides the NULL terminator
	lbu $t0, 0($v1)
	beq $t0, $0, write_machine_code_done
	
	# 3. Looks like there is another instruction. Call parse_int() to convert
	# the instruction into a numerical format, and store it into a register:
	# YOUR_INSTRUCTIONS_HERE 
	move $a0, $v1
	li $a1, 16
	jal parse_int
	move $s6, $v0			# s6 = instruction in hex
	# END_INSTRUCTIONS_HERE
	...
	beq $v0, $0, write_machine_code_to_file	# write directly if no need for relocation
	# END_INSTRUCTIONS_HERE
	...
	# YOUR_INSTRUCTIONS_HERE
	move $a0, $s6			# $a0 = $s6 = instruction
	move $a1, $s5			# $a1 = $s5 = byte offset
	move $a2, $s2
	move $a3, $s3
	jal relocate_inst
	move $s6, $v0			# $6 = the relocated inst
	# END_INSTRUCTIONS_HERE
write_machine_code_to_file:
	...
	# YOUR_INSTRUCTIONS_HERE
	addiu $s5, $s5, 4			# increment byte offset
	# END_INSTRUCTIONS_HERE
	# Here, we use the write to file syscall, passing in the string buffer
	move $a0, $s0
	la $a1, hex_buffer
	li $a2, 9				# 8 digits + newline
	li $v0, 15
	syscall
