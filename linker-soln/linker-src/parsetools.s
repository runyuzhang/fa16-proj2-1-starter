...
# be writing 10 characters (8 digits, 1 newline, 1 NULL-terminator).
#
...
#------------------------------------------------------------------------------
hex_to_str:
	li $t0, 7		# counter for digits
hex_to_str_loop:
	sll $t1, $t0, 2	# convert to # bits to shift
	srlv $t2, $a0, $t1	# bring byte of interest to right
	andi $t3, $t2, 0xF	# mask off everything else
	bgt $t3, 9, hex_to_str_hex
	addiu $t3, $t3, 48	# convert 0-9 to char
	j hex_to_str_write
hex_to_str_hex:	
	addiu $t3, $t3, 87	# convert a-f to char
hex_to_str_write:
	sb $t3, 0($a1)
	addiu $a1, $a1, 1
	addiu $t0, $t0, -1
	bge $t0, $0, hex_to_str_loop
	# write \n\0
	li $t0, 10
	sb $t0, 0($a1)
	sb $0, 1($a1)
