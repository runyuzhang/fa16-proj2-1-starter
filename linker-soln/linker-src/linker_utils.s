...
# 2) the symbol name is not in the symbol table. In that case, return -1.
# You may assume that besides those two issues, no other errors will occur 
...
relocate_inst:
	...
	lw $a0, 4($sp)
	move $a1, $v0
	jal addr_for_symbol				# checks symbol table
	li $t0, -1
	beq $v0, $t0, relocate_inst_error
	
	lw $a0, 12($sp)	# $a0 = instruction
	lui $t0, 0xFC00
	and $t3, $a0, $t0	# $t3 = opcode
	srl $v0, $v0, 2	# discard bottom 2 bits
	li $t0, 0x03FFFFFF	# consider what specific type of instructions go here
	and $t4, $v0, $t0
	or $v0, $t3, $t4
	j relocate_inst_end
relocate_inst_error:
